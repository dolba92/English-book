...
export function SettingsPage() {
  ...
  const handleClearAll = async () => {
    if (confirm("WARNING: This will delete ALL books, progress, and words. Are you absolutely sure?")) {
      await clearAllData();
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-6 md:p-10 max-w-3xl mx-auto min-h-screen">
      ...
      <div className="space-y-10">
        {/* Theme */}
        <section>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2">Theme</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { id: 'light', name: 'Light', bg: 'bg-[#faf9f6]', border: 'border-gray-200', text: 'text-gray-900' },
              { id: 'pink', name: 'Love Texas (Default)', bg: 'bg-[#fff5f6]', border: 'border-pink-200', text: 'text-pink-950' },
              ...
              >
                ...
                <div className="space-y-2 opacity-70">
                  <div className={`h-2 rounded-full w-full ${t.text} bg-current`}></div>
                  <div className={`h-2 rounded-full w-2/3 ${t.text} bg-current`}></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <select 
                ...
                className="bg-muted border-none rounded-xl px-4 py-2 text-sm focus:ring-2 focus:ring-primary outline-none"
              >
